...
import * as fButton from '../floating-button/floating-button'
...
import * as bButton from '../block-button/block-button'
...
export const view = (d: EventEmitter, model: TrackModalModel) => {
  ...
  return h('div.track-modal-content', [
    ...
    h('div.floating-button-container', {on: {click: [playEE.listen, track]}}, [
      fButton.view(icon(model), model.audio.status === MediaStatus.LOADING)
    ]),
    h('div.track-info', [
      h('div.header', [
        h('div', [
      ...
      h('div', [
        bButton.view({on: {click: [playEE.listen, track]}}, ['play']),
        bButton.view({}, ['enqueue'])
      ])
    ])
  ])
}
